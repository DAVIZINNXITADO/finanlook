...
import { ArrowRight, ChartPie, LifeBuoy, Sparkles, Target, Wallet } from "lucide-react";
import { Button } from "@/components/ui/button";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "FinanFácil — organização financeira simples" },
      {
        name: "description",
        content:
          "Registre entradas e gastos, organize seu salário, monte sua reserva de emergência e acompanhe metas. Sem termos complicados.",
      },
      { property: "og:title", content: "FinanFácil — organização financeira simples" },
      {
...
const FEATURES = [
  { icon: Wallet, title: "Entradas e gastos", text: "Registre em segundos e veja seu saldo real." },
  { icon: ChartPie, title: "Gráficos claros", text: "Descubra para onde seu dinheiro está indo." },
  { icon: LifeBuoy, title: "Reserva de emergência", text: "Monte sua proteção aos poucos." },
  { icon: Target, title: "Metas", text: "Notebook, viagem, curso — acompanhe o progresso." },
];
      ...
      <main className="mx-auto w-full max-w-5xl px-5 pb-16">
        <section className="surface hero-gradient overflow-hidden px-6 py-12 text-primary-foreground sm:px-10 sm:py-16">
          <p className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1 text-xs font-medium">
            <Sparkles className="size-3.5" /> Simples de verdade
          </p>
          ...
          <p className="mt-4 max-w-xl text-sm/relaxed opacity-90 sm:text-base">
            O FinanFácil ajuda você a saber quanto entra, quanto sai, quanto sobra e quanto você
            está conseguindo guardar — tudo em uma tela só.
          </p>
        ...
        <p className="mt-8 text-center text-xs text-muted-foreground">
          O FinanFácil é uma ferramenta de organização pessoal. Não vendemos investimentos nem
