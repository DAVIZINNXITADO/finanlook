...
export const Route = createFileRoute("/_authenticated/reserva")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Minha reserva — FinanLook" },
...
function ReservePage() {
  ...
  useEffect(() => {
    ...
    setCurrent(
      reserve?.current_amount
        ? String(Number(reserve.current_amount)).replace(".", ",")
        : "",
    );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Minha reserva"
        subtitle="Sua proteção para imprevistos."
      />

      <p className="rounded-2xl bg-accent/60 px-4 py-3 text-sm">
        🛟 Uma reserva de emergência é um valor separado para ajudar a lidar
      ...
      <section className="surface p-5">
        ...
        >
          {save.isPending ? "Salvando..." : "Salvar reserva"}
        </Button>
      </section>
      ...
      <section className="surface p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Movimentações de saída na categoria “Reserva de emergência”.
          Total:{" "}
        ...
        <p className="mt-4 text-xs text-muted-foreground">
          O FinanLook é apenas uma ferramenta de organização. Não vendemos
