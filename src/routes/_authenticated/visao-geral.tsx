...
function Overview() {
  ...
  useEffect(() => {
    document.title = "Visão geral — FinanFácil";
  }, []);
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-xs font-medium text-muted-foreground">{monthLabel(month)}</p>
      </header>
      ...
      <section className="surface p-5">
        <h2 className="font-display text-lg font-semibold">Onde seu dinheiro está indo?</h2>
        {isLoading ? (
            ...
            <EmptyState
              ...
              action={
                <Button asChild className="mt-2">
                  <Link to="/movimentacoes">Adicionar movimentação</Link>
                </Button>
              }
            />
          </div>
              ...
              {summary.byCategory.map((item, i) => (
                <li key={item.category} className="flex items-center gap-3 text-sm">
                  ...
                  <span className="w-10 text-right text-xs text-muted-foreground">
                    {summary.expenses > 0 ? Math.round((item.total / summary.expenses) * 100) : 0}%
                  </span>
                </li>
              ))}
            </ul>
      ...
      {topExpenses.length > 0 ? (
        <section className="surface p-5">
          <h2 className="font-display text-lg font-semibold">Maiores gastos do mês</h2>
              ...
              <li key={t.id} className="flex items-center justify-between gap-3 py-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {CATEGORY_EMOJI[t.category] ?? "•"} {t.category} · {formatDateBR(t.date)}
                  </p>
                </div>
                <span className="shrink-0 text-sm font-semibold">{formatBRL(t.amount)}</span>
              </li>
            ))}
          </ul>
        </section>
      ...
      <section className="surface p-5">
        <h2 className="flex items-center gap-2 font-display text-lg font-semibold">
          <Sparkles className="size-4 text-primary" /> Insights financeiros
        </h2>
        {insights.length === 0 ? (
          <p className="mt-3 text-sm text-muted-foreground">
            Assim que você registrar algumas movimentações, mostramos observações sobre seus
            hábitos aqui.
          </p>
        ) : (
          <ul className="mt-3 space-y-2">
            {insights.map((text) => (
              <li key={text} className="rounded-xl bg-accent/50 px-4 py-3 text-sm">
                {text}
              </li>
            ))}
          </ul>
